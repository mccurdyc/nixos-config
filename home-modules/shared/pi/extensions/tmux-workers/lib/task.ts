...
export function buildWorkerTask(task: string, resultFile: string, workerDir: string): string {
	return [
		...
		"- NEVER write to /tmp/ — use your worker directory for ALL file output",
		"- If the task asks you to write findings/summary to a file, write it",
		`  inside your worker directory: ${workerDir}/<filename>`,
