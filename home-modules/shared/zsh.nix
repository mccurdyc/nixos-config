{ pkgs, lib, ... }: {
  ...
  programs.zsh = {
    ...
    shellAliases = {
      ...
      g = ''nvim -c 'Neogit' -c 'autocmd WinClosed,BufEnter * if winnr("$") == 1 && empty(expand("%")) && !&modified && getline(1, "$") == [""] | qall | endif''';
      gits = "git status";
      gitfc = ''
        (){ git log --format = format: "%H" | tail - 1; } '';
      kubectl_pods_containers = ''
          kubectl
          get
          pods - o jsonpath='{
